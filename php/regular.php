...
addBr('Даны строки с датами, в которых год может изменяться от 1990 до 2099:<br/>	$arr' . " = [<br/>
		...
		'30-11-1995',<br/>
		'29-10-1990',<br/>
	];<br/>" . ' Для каждой даты положите день в первый карман, месяц во второй, а последние две цифры года - в третий.');
$arr = [
	'31-12-2025',
	'30-11-1995',
	'29-10-1990',
...
addBr('');
addBr(aC("
" . '$reg' . " = '#(\d+)-(\d+)-(?|19(\d{2})|20(\d{2}))#';<br/>
foreach (" . '$arr' . " as " . '$k' . " => " . '$v' . ") {<br/>
	preg_match(" . '$reg' . ", " . '$v' . ", " . '$match' . ");<br/>
	echo'day: '. " . '$match[1]' . " .' month: '. " . '$match[2]' . ".' year: '. " . '$match[3];' . "<br/>
}<br/>
"));
$reg = '#(\d+)-(\d+)-(?|19(\d{2})|20(\d{2}))#';
foreach ($arr as $k => $v) {
...
addBr(rB('Позитивный и негативный просмотр в PHP'));
addBr("Иногда нужно решить задачу такого типа: найти строку 'aaa' и заменить ее на '!', но только если после 'aaa' стоит 'x', а сам 'x' при этом не заменять. Если мы попытаемся решить задачу 'в лоб', то у нас ничего не выйдет:");

addBr(aC("preg_replace('#aaax#', '!', 'aaax')") . ' = ' . preg_replace('#aaax#', '!', 'aaax') . " ...А нам надо '!x'");
addBr(rB('Просмотр вперед'));
addBr("Для решения задачи нужен способ сказать, что 'x' не следует заменять. Делается это с помощью специальных скобок " . rB('(?= )') . ", которые просто смотрят, но не забирают с собой.
...
addBr(aC("preg_replace('#aaa(?=x)#', '!', 'aaax')") . ' = ' . preg_replace('#aaa(?=x)#', '!', 'aaax') . " ...то, что нам надо!");
addBr("Есть и негативный просмотр вперед - " . rB('(?! )') . " - он, наоборот, говорит, что чего-то должно не быть. В следующем примере замена произойдет, только если после 'aaa' не стоит 'x':");
addBr(aC("preg_replace('#aaa(?!x)#', '!', 'aaab')") . ' = ' . preg_replace('#aaa(?!x)#', '!', 'aaab'));
addBr('');
addBr(rB('Просмотр назад'));
addBr("Аналогичным образом есть позитивный просмотр назад - " . rB('(?<= )') . ". В следующем примере замена произойдет, только если перед 'aaa' стоит 'x':");
addBr(aC("preg_replace('#(?<=x)aaa#', '!', 'xaaa')") . ' = ' . preg_replace('#(?<=x)aaa#', '!', 'xaaa'));
addBr("И есть также негативный просмотр назад - " . rB('(?' . htmlspecialchars('<') . '! )') . " В следующем примере замена произойдет, только если перед 'aaa' не стоит 'x':");
addBr(aC("preg_replace('#(?" . htmlspecialchars('<') . "!x)aaa#', '!', 'baaa')") . ' = ' . preg_replace('#(?<!x)aaa#', '!', 'baaa'));
addBr('');
addBr(rB("Практические задачи"));
addBr('Дана строка, содержащая имена функций: $str = ' . "'func1() func2() func3()';" . " Получите массив имен функций из строки.");
$str = 'func1() func2() func3()';
$reg = '#\w+(?=())#';
preg_match_all($reg, $str, $match);
echo (aC(
'$str' . " = 'func1() func2() func3()';<br/>" .
'$reg' . " = '#\w+(?=())#';<br/>" . "
preg_match_all(" . '$reg' . ", " . '$str' . ", " . '$match' . ");<br/>
var_dump(" . '$match[0]' . "); 
"
)
. ' = ');
...
addBr('');
addBr('');
addBr('Дана строка с тегом: $str = ' . htmlspecialchars('<a href="" class="eee" id="zzz">;') . " Получите массив имен атрибутов этого тега.");
...
addBr('');
addBr('');
addBr('Дана строка с переменными: $str = ' . '$aaa $bbb $ccc' . " Получите массив имен переменных из этой строки (без доллара).");
$str = '$aaa $bbb $ccc';
$reg = '#(?<=\$)\w+#';
preg_match_all($reg, $str, $match);
echo (aC(
...
addBr(rB('Замена с коллбэком через регулярки в PHP'));
addBr('Пусть у нас есть следующая строка:');
addBr(aC('$str'." = '2+3= 3+5= 7+8=';"));
addBr('Пусть нам нужно найти все конструкции вида число+число= и сделать так, чтобы после равно появился результат сложения.
...
addBr(aC('	$str'." = '2+3= 3+5= 7+8=';".'<br/>
	preg_replace_callback'."('#(\d+)\+(\d+)=#', function(".'$match'.") {<br/>
		var_dump(".'$match'.");<br/>
	}, ".'$str);'));
addBr('В результате наш var_dump сработает три раза, по очереди выведя следующее:');
addBr(aC("['2+3=', '2', '3']<br/>
['3+5=', '3', '5']<br/>
['7+8=', '7', '7']"));
addBr("Идем дальше. Каждое совпадение будет заменено на то, что возвращает коллбэк через return. Давайте для примера заменим каждую из искомых подстрок на знак '!':");
addBr(aC('	$str'." = '2+3= 3+5= 7+8=';<br/>
	
	".'$res'." = preg_replace_callback('#(\d+)\+(\d+)=#', function(".'$match'. ") {<br/>
		return '!';<br/>
	}, ".'$str'. ");<br/>
	
	echo ".'$res'.";")." // выведет '! ! !'");
addBr('А теперь заменим каждую подстроку на сумму пары чисел в ней:');
addBr(aC('	$str'." = '2+3= 3+5= 7+8=';<br/>
...
addBr('Получается, что наша задача почти решена. Осталось сделать так, чтобы перед суммой чисел оставалась искомая подстрока. Для этого будем вставлять перед суммой содержимое нулевого кармана:');
addBr(aC('	$str'." = '2+3= 3+5= 7+8=';<br/>
...
addBr('');
addBr('Дана строка с целыми числами. С помощью регулярки преобразуйте строку так, чтобы вместо этих чисел стояли их квадраты.');
$str = '2 3 4';
$res = preg_replace_callback('#\d#', function($match){
	return pow($match[0],2);
}, $str);
echo(aC('	$str' . " = '2 3 7';<br/>
".'$res'. "= preg_replace_callback('#\d#', function(" . '$match' . ") {<br/>
return pow(" . '$match[0],2)' . ";<br/>
}, " . '$str' . ");<br/>".'
echo $res').' = ');
echo $res;
hr();
...
addBr(rB('Модификаторы регулярок в PHP'));
addBr('Как вы уже знаете, модификаторы представляют собой команды, которые пишутся после правого ограничителя регулярки. Давайте посмотрим, что полезного можно сделать с их помощью.');
addBr(rB('Игнорирование регистра регулярок в PHP'));
addBr('По умолчанию в регулярках регистр символов имеет значение. В следующем примере мы найдем только буквы в нижнем регистре:');
addBr(aC("preg_replace('#[a-z]+#', '!', 'aaa bbb AAA');")."// вернет '! ! AAA'");
addBr('Мы можем изменить такое поведение регулярки, использовав модификатор i. Исправим нашу регулярку в соответствии с этим:');
addBr(aC("preg_replace('#[a-z]+#i', '!', 'aaa bbb AAA');")."// вернет '! ! !'");
addBr(rB('Задача'));
addBr('Упростите следующий код, использовав соответствующий модификатор:');
addBr("preg_replace('#[a-zA-Z]+#', '!', 'aaa BBB');");
addBr(aC("preg_replace('#[a-z]+#i', '!', 'aaa BBB');"));
hr();


addBr(rB('Комментарии в регулярке в PHP'));
addBr("Модификатор x разрешает расставлять в регулярке произвольные пробелы и переводы строк. Кроме того, символ решетки становится символом однострочного комментария (соотвественно, ограничители придется заменить на другие).<br/>
Давайте посмотрим на примере. Пусть у нас есть вот такая регулярка:");
addBr(aC("preg_replace('#[a-z]+@[0-9]+#', '!', 'aaa@333');"));
addBr("Давайте используем модификатор x и расставим в для каждой части нашей регулярки поясняющие комментарии:");
addBr(aC("	preg_replace('~<br/>
[a-z]+ # буквы <br/>
@      # символ собаки<br/>
[0-9]+ # цифры<br/>
~x', '!', 'aaa@333');"));
...
addBr(rB('Работа с юникодом в регулярках в PHP'));
addBr('Модификатор u, с которым вы уже сталкивались ранее, заставляет регулярку корректно работать с юникодом. В частности, корректно обрабатывать кириллические строки:');
addBr(aC("	preg_replace('#[а-яё]#u', '!', 'яяя ййй ёёё');"));
hr();


addBr(rB('Изменение жадности регулярок в PHP'));
addBr('Модификатор U делает так, что все операторы повторений станут по умолчанию нежадными, а добавление ?, наоборот, будет добавлять им жадности:');
addBr(aC("	preg_replace('#x.+x#U', '!', 'xax xaax xaaax'); // вернет '! ! !'"));
hr();


addBr(rB('Экранировка обычных символов в регулярках в PHP'));
addBr("Модификатор X делает так, что экранировка обычного символа приведет к ошибке. Без этого оператора обычные символы можно экранировать, они все равно будут обозначать сами себя (исключение: цифры, они станут карманами). Посмотрим на примере:");
addBr(aC("	preg_replace('#x\@x#X', '!', 'x@x'); // ошибка, @ не спецсимвол"));
hr();


addBr(rB('Несколько модификаторов регулярок в PHP'));
addBr("После ограничителя можно писать несколько модификаторов. Пример:");
addBr(aC("preg_replace('#[a-z]+#iU', '!', 'aaa bbb');"));

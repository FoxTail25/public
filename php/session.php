...
<h3 class="fw-bold mt-5">Запись массивов в сессии PHP</h3>
<p>Рассмотрим код, сделанный нами в предыдущем уроке:</p>
<code>
	<pre>
		&lt?php
		if (!empty($_GET)) {
...
<p>На самом деле можно было взять и записать все данные формы сразу в $_SESSION:</p>
<code>
	<pre>
		&lt?php
		if (!empty($_GET)) {
...
<p>А в файле test2.php сделать вот так:</p>
<code>
	<pre>
		&lt?php
		if (!empty($_SESSION)) {
...
<p>Преимущество такого подхода в том, что наш код будет работать независимо от количества инпутов в форме.
	<br />Есть, однако, и недостаток: перезаписывая $_SESSION мы затираем все данные, которые там ранее были. Мало ли что туда написал другой наш скрипт? А мы это удалим.
	<br />Давайте поступим по-другому:
</p>
<code>
	<pre>
		&lt?php
...
<p>Как вы видите, в переменную сессии мы записали не одно какое-то значение, а целый массив. Теперь в файле test2.php можем найти сумму элементов этого массива:</p>
<code>
	<pre>
		&lt?php
		if (!empty($_SESSION)) {
...
<p class="fw-bold mt-5">Задача:</p>
<p>На одной странице с помощью формы спросите у пользователя имя, возраст, зарплату и еще что-нибудь. Запишите эти данные в одну переменную сессии в виде массива. При заходе на другую страницу переберите сохраненные данные циклом и выведите каждый элемент массива в своем теге li тега ul.</p>

<p class="fw-bold"><a href="/session/sess_form_z.php">Решение</a></p>